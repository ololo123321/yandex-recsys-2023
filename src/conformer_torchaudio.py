...
class Conformer(torch.nn.Module):
    ...
    *Conformer: Convolution-augmented Transformer for Speech Recognition*
    ...
    def __init__(
        ...
        ffn_dim: int = 1024,
        num_layers: int = 6,
        depthwise_conv_kernel_size: int = 31,  # обязан быть нечётным
        dropout: float = 0.0,
        num_classes: int = 256,
        use_group_norm: bool = False,
        convolution_first: bool = False,
    ...
    def forward(self, x, mask, **kwargs) -> Tuple[torch.Tensor, torch.Tensor]:
        if self.fc_in is not None:
            x = self.fc_in(x)
        x = x.transpose(0, 1)  # [T, N, D]
        key_padding_mask = torch.logical_not(mask.bool())
        for layer in self.conformer_layers:
            x = layer(x, key_padding_mask)
        x = x.transpose(0, 1)  # N, T, D
